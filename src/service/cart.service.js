...
const addProductToCart = async (productInfo,user)=>{
    ...
    if(!cart){
        cart = await CartModel.create({
            cartItems: [{product: product._id, price:product.price, quantity: productInfo.quantity}],
            user: user._id
        })
    }else {
...
export const updateItemQuantity = async (productInfo, productId, userId)=>{
    ...
    const cartItemIndex = cart.cartItems.findIndex((item)=> item._id.toString() === productId)
